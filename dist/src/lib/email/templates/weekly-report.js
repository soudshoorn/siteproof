...
function weeklyReportEmail(data) {
    const avgScore = data.websites.filter((w) => w.latestScore != null).length > 0
        ? Math.round(data.websites
    ...
    const totalCritical = data.websites.reduce((sum, w) => sum + w.criticalIssues, 0);
    const websiteRows = data.websites
        .map((website) => {
        const scoreColor = website.latestScore == null
            ? "#666"
            : website.latestScore >= 80
                ...
                : website.latestScore >= 50
                    ? "#eab308"
                    : "#ef4444";
        const trend = website.previousScore != null && website.latestScore != null
    ...
    ${(0, base_1.emailButton)("Bekijk dashboard", `${APP_URL}/dashboard`)}
  `;
    return {
