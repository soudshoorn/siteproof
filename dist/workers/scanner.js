...
const crawler_1 = require("../src/lib/scanner/crawler");
const analyzer_1 = require("../src/lib/scanner/analyzer");
const score_1 = require("../src/lib/scanner/score");
const notifications_1 = require("../src/lib/email/notifications");
...
async function processScanJob(data) {
    ...
    try {
        ...
        const crawlResult = await (0, crawler_1.crawlWebsite)(activeBrowser, url, maxPages, async (scanned, total) => {
            await prisma.scan.update({
                where: { id: scanId },
        ...
        for (let i = 0; i < pagesToScan.length; i++) {
            ...
            try {
                const analysis = await (0, analyzer_1.analyzePage)(activeBrowser, pageUrl);
                // Save PageResult
                ...
                if (error instanceof analyzer_1.PageAnalysisError) {
        ...
        const overallScore = scoredPages.length > 0
            ? (0, score_1.calculateOverallScore)(scoredPages.map((p) => ({ score: p.score, issueCount: p.issueCount })))
            : 0;
        ...
        console.log(`[Scanner] Scan ${scanId} completed: score=${overallScore}, ` +
        ...
        try {
            await (0, notifications_1.notifyScanCompleted)(prisma, scanId);
        }
        catch (emailError) {
 ...
 */
async function processQuickScanJob(data) {
    ...
    try {
        ...
        const activeBrowser = await getBrowser();
        const analysis = await (0, analyzer_1.analyzePage)(activeBrowser, url);
        ...
        console.log(`[Scanner] Quick scan ${quickScanId} completed: score=${analysis.score}, issues=${analysis.issues.length}`);
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
...
const worker = new bullmq_1.Worker("scans", async (job) => {
    if (job.name === "quick-scan") {
        await processQuickScanJob(job.data);
    }
    else {
